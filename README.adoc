...
The project **connector-starwars** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/connector-starwars.git
cd connector-starwars/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `connector-starwars-1.0-SNAPSHOT.jar`
